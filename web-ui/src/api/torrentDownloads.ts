...
export function getTorrentSource(infoHash: string, user: User): Promise<{ value: string }> {
  return apiRequest<{ value: string }>(`/torrent-download/by-hash/${infoHash}/torrent-source`, user);
}
