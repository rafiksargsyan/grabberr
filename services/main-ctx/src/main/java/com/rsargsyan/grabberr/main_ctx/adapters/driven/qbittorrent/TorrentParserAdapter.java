...
@Component
public class TorrentParserAdapter implements TorrentParser {

  @Override
  public String parseInfoHash(String downloadUrl) {
    if (downloadUrl == null || downloadUrl.isBlank()) throw new InvalidDownloadUrlException();
  ...
  @Override
  public String parseInfoHash(byte[] torrentFileBytes) {
    try {
  ...
  private String parseMagnetInfoHash(String magnet) {
  ...
  private String base32ToHex(String base32) {
    byte[] bytes = new Base32().decode(base32.toUpperCase());
    return HexFormat.of().formatHex(bytes);
  }

  // --- Minimal bencode parser ---

  private byte[] extractInfoDictBytes(byte[] data) {
    if (data[0] != 'd') throw new InvalidDownloadUrlException();
    int pos = 1;
  ...
  private int skipValue(byte[] data, int pos) {
    byte b = data[pos];
    if (b == 'd' || b == 'l') {
      pos++;
  ...
  // returns [dataStart, dataEnd]
  private int[] readString(byte[] data, int pos) {
  ...
  private int findByte(byte[] data, byte target, int from) {
